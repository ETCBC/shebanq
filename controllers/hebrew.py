...
def text():
    ...
    book_name = {}
    for vr in versions:
        if not versions[vr]['present']: continue
...
def material():
    ...
    return from_cache(
        'verses_{}_{}_{}_{}_{}_{}_{}_'.format(vr, mrrep, bk if mr=='m' else iidrep, ch if mr=='m' else page, tp, tr, lang),
...
def material_c(vr, mr, qw, bk, iidrep, ch, page, tp, tr, lang):
    if mr == 'm':
        ...
        material = Verses(passage_dbs, vr, mr, chapter=chapter['id'], tp=tp, tr=tr, lang=lang) if chapter else None
        result = dict(
            mr=mr,
            qw=qw,
            hits=0,
...
'''.format(
    ...
    if records == None:
        ...
        for (nid, uid, ufname, ulname, ca, shared, pub, pub_on, status, keywords, ntext) in records:
            ...
            ro = myid == None or uid != myid or not edit or (pub and pub_on != None and (pub_on <= request.utcnow - PUBLISH_FREEZE))
...
'''.format(
    ...
    u"'{}'".format(request.utcnow) if pub and vr != CONTINUOUS else 'null',
...
def load_notes(uid, filetext, msgs):
    ...
    chunksize = 100
    sqlhead = u'''
insert into note ({}, created_by, created_on, modified_on, shared_on, published_on, bulk) values
'''.format(u', '.join(fieldnames))
    ...
    for (i, linenl) in enumerate(unicode(filetext.value, encoding='utf8').split(u'\n')):
        ...
        if len(keywordList) == 0:
            errors.setdefault('empty keyword', []).append('{}:"{}"'.format(i+1, keywords))
            continue
        allKeywords |= set(keywordList)
        keywords = ''.join(' {} '.format(x) for x in keywordList)
        allVersions.add(version)
        this_chunk.append(u"('{}','{}',{},{},{},'{}','{}','{}','{}','{}',{},'{}','{}',{},{},'b')".format(
    ...
    if errors or nerrors:
        ...
        whereKeywords = ' or '.join(" keywords like '% {} %' ".format(keyword) for keyword in keywordList)
        # first delete previously bulk uploaded notes by this author and with these keywords and these versions
        delSql = "delete from note where bulk = 'b' and created_by = {} and {} and {};".format(uid, whereVersion, whereKeywords)
...
'''.format(qid, vr)
                ...
                if uptodate != 'good':
                    ...
                    break
                if vr == CONTINUOUS:
                    msgs.append(('error', u'You can only publish if the query results on a fixed version of the data. Version "{}" may change.'.format(CONTINUOUS)))
                    break
                sql = u'''
...
def query_fields(vr, q_record, recordx, single_version=False):
    ...
    if not single_version:
        q_record['versions'] = dict((v, dict(
            ...
            eversion=None,
            published_on=None,
        )) for v in versions if versions[v]['present'])
        for rx in recordx:
            vx = rx['version']
